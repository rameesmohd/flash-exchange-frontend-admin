...
import managerAxios from '../axios/managerAxios'
const axiosInstance = managerAxios()
import { store } from "../redux/Store";
import { jwtDecode } from "jwt-decode";
import { managerLogout } from '../redux/ManagerSlice';

const getToken = () => {
    const storedUser = localStorage.getItem("persist:Manager");
...
const managerGet = async (url, options = {}) => {
    try {
...
const managerPost = async (url, formData) => {
    try {
      const token = getToken();
      if (token && isTokenExpired(token)) {
        message.error("Session expired. Please log in again.");
...
const managerPatch = async (url, formData, img) => {
    try {
      const token = getToken();
      if (token && isTokenExpired(token)) {
        message.error("Session expired. Please log in again.");
...
const managerDelete = async (url) => {
  ...
  const logoutUser = () => {
    ...
    window.location.href = "/manager/login"; // Uncomment if redirection is needed
  };
